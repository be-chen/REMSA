...
def strip_markdown_fences(text: str) -> str:
    """
    Extracts the first JSON object found between balanced braces.
    Ignores markdown fences and any trailing notes.
    """
    ...
    raise ValueError("No matching closing brace '}' found for the JSON object.")
...
class ChatGPT(BaseEngine):
    def __init__(self, model_name_or_path: str, api_key: str, base_url=openai.base_url):
        ...
        self.temperature = 0.01 #0.2
        self.top_p = 1 #0.9
        self.max_tokens = 6000 # 4096 Close source model
        if api_key != "":
    ...
    def get_chat_response(self, input):
        ...
        while retries < 10:
            try:
                ...
                print("]]]]]]]]]]]]]]]]]")
                print(raw)
                # Clean only the Markdown fences
                cleaned_text = strip_markdown_fences(raw)

                # Replace the content before parsing
                response.choices[0].message.content = cleaned_text
                print(cleaned_text)
                response = add_logprobs(response)
                logp_value = response.log_probs[0]
                return cleaned_text, logp_value
