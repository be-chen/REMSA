...
# Now please extract the corresponding information from the text. Ensure that the information you extract has a clear reference in the given text or can be clearly inferred or caculated from the reference. Set any property that you are not sure about the value to null.
# Return **only raw JSON format STRING**.
# Do NOT include any markdown fences (```) or wrapper text.
# Do NOT say anything else—no explanations, no commentary.
# """

EXTRACT_INSTRUCTION = """
**Instruction**: You are an agent skilled in information extarction. {instruction}
Please extract the corresponding information from the given text based on the output schema below. Ensure that the information you extract has a clear reference in the given text or can be clearly inferred or caculated from the reference. Set any property that you are not sure about the value to null.
Return **only raw JSON format STRING**.
Do NOT include any markdown fences (```) or wrapper text.
Do NOT say anything else—no explanations, no commentary.
When outputting numbers, write them as continuous digits without any commas or separators.
{examples}
**Output Schema**: {schema}
**Text**: {text}
{additional_info}
...
Return **only raw JSON format STRING**.
Do NOT include any markdown fences (```) or wrapper text.
Do NOT say anything else—no explanations, no commentary, no Note!
When outputting numbers, write them as continuous digits without any commas or separators.
{examples}
**Task**: {instruction}
